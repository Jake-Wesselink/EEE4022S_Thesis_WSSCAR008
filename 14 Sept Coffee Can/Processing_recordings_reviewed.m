...
wavFile_CW_All = {'2cars_differentSpeed.wav';
                  'vennom_speedingUp.wav';
                  'Bakkie_60kph.wav';
                  'vennom_30kph_new.wav';
                  'vennom_rangeTowards_0m.wav'};
...
multi = 10;
detectionThreshold = multi*max(abs(S_OfInterest(:,1))); 
...
%entire matrix 
%S_OfInterestToPlot = 2*abs(S_OfInterest) / max(max(abs(S_OfInterest)));
