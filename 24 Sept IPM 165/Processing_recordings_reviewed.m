...
wavFile_CW_All = {'165_Bakkie_60km_toward_clipped.wav';
                  '165_Bakkie_RangeAway2.wav';
                  '165_Bakkie_RangeTowards.wav';
                  '165_Bakkie_slowingdown_toward.wav';
                  '165_Bakkie_speedingup_towards.wav';
                  '165_car_30kmph.wav';
                  '165_car_towards.wav';
                  '165_car_rangeTowards.wav';
...
%        % If the max value in the column is above the threshold, normalise
%        %disp(maxVal);
%        S_OfInterestToPlot(:, i) = 2*abs(S_OfInterest_mod(:, i)) / maxVal;
%    else
%        % If below threshold, set column values to zero or some lower intensity
%        S_OfInterestToPlot(:, i) = abs(S_OfInterest_mod(:,i))/(1*detectionThreshold);  
%    end
% end
